...
class Editor {
  ...
  var completionIndex : Int;
	...
	//追加部分
	var con = new Http("http://localhost/haxeon_first/htdocs/userinfo.php");
	con.onError = onError;
	con.onData = onResult;

	con.request(false);
	trace(this.tmpResult);
	//session = new JQuery(".session");
	//session.html(tmpResult);
